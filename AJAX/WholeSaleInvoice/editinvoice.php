...
if(isset($_POST["items"]))
{
    ...
    if($product[0]["ItemType"]=="P")
    {
        if(count($batch)==0)
        {
            ...
            if($minus==1)
            {
                ...
                $batch2=$wholesale_invoice->select_batch_with_pro_id(pro_id: $pro_id,shop_id: $shop_id,minus: $minus,company:$com_id);
                if(count($batch2)==0)
                {
                    ?>
                    <script>
    ...
    {
        $batch2=$wholesale_invoice->select_batch_with_pro_id($pro_id,$shop_id,1);
        if(count($batch2)==0)
        {
            ?>
            <script>
