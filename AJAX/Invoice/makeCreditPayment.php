    ...
    if(qtyCheck($sell_header_id, $shop_id))
    {
        if($paid_amount < $net_total)
        {
            if(expireCheck($sell_header_id, $shop_id))
            {
                ...
                //create invoice
                $invoice_header_id = createInvoice($sell_header_id, $cust_pay, $pending_amount, $user_id, $customer_id, $salesman_id, $shop_id, $counter_id);
        ...
        {
            //already fully paid via multipay - do not re-process
            $invoice_stat = 5;
        }//already paid
    }//qty check
}//user invoice limit
...
function createInvoice($sell_header_id, $cust_pay, $balance_payment, $user_id, $customer_id, $salesman_id, $shop_id, $counter_id)
{
    ...
    CreateCreditCustomer($sell_header_id, $next_header_id, $customer_id, $user_id);
