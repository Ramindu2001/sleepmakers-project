        ...
        setTimeout(function(){
            ...
            if(isset($_SESSION["deliveryNote"]))    
            {
                ...
                else
                {
                    unset($_SESSION["deliveryNote"]);
                    ?>
                    window.location="../Public/gui-pos.php";
                    <?php
                }
                unset($_SESSION["deliveryNote"]);
            }
...
<head>
    ...
    <title><?=$shop_name." - ".$invoiceNo?></title>
...
<body>
    ...
    <div id="div_cart">
        <table id="tbl_cart">
            ...
            foreach($invData as $row)
            {
                ...
                if($disc_type==1)
                {
                    // if it is percentage discount
                    $discount=($sell_qty * $unit_price)* $PercentDiscount/100;
                }
                else
                {
                    $discount=$sell_qty*$DirectDiscount;
                }
                ...
                <tr style="border-bottom: 1px solid black;">
                    ...
                    <td  style="text-align: center;"><?php echo $sell_qty;?></td>
                    <td ><?php echo $unit_price;?></td>
                    <td class="td_total"><?php echo $row['SoldAmount'];?></td>
                </tr>
                <?php 
            }//foreach
            ?>
    ...
    <div>
        ...
        //get discounts (computed from authoritative header fields so the values
        //always match what was charged, regardless of the DiscountAmount column).
        // - lineDiscount   : total per-product (line) discount stored on the header.
        // - discountType   : invoice-level discount type (1 = percentage, 2 = flat).
        // - PercentDiscount: invoice-level percentage value (when discountType = 1).
        // - FixedDiscount  : invoice-level flat amount (when discountType = 2).
        $gross_for_disc       = floatval($headerData[0]['GrossAmount']);
        $header_line_discount = floatval($headerData[0]['lineDiscount']);
        $header_disc_type     = isset($headerData[0]['discountType']) ? $headerData[0]['discountType'] : 2;
        $header_fixed_disc    = floatval($headerData[0]['FixedDiscount']);
        $header_percent_disc  = floatval($headerData[0]['PercentDiscount']);

        if($header_disc_type == 1)
        {
            // percentage invoice discount is applied on the gross amount
            $invoice_discount = $gross_for_disc * $header_percent_disc / 100;
        }
        else
        {
            // flat invoice discount
            $invoice_discount = $header_fixed_disc;
        }
        if($invoice_discount < 0){ $invoice_discount = 0; }

        // Prefer the stored line discount; fall back to the value recomputed in the
        // cart loop above if the header column is empty.
        if($header_line_discount > 0){ $line_discount = $header_line_discount; }
        ...
        <tr style="font-size: 14px;">
            ...
            <td style="text-align: right;"><?php echo number_format((float)$gross_amount, 2, '.', '');?></td>
        </tr>
        ...
        if($line_discount > 0)
        {
            ...
            <tr style="font-size: 14px;">
                ...
                <td style="text-align: right;"><?php echo number_format($line_discount, 2, '.', '');?></td>
            </tr>
            <?php 
        }//has item (line) discount
        ...
        if($invoice_discount > 0)
        {
            ...
            <tr style="font-size: 14px;">
                ...
                <td style="text-align: right;"><?php echo number_format($invoice_discount, 2, '.', '');?></td>
            </tr>
            <?php 
        ...
        if($total_discount > 0)
        {
            ...
            <tr style="font-size: 14px; font-weight:bold;">
                <td style="text-align: right; border-top:1px solid #000;">Total Discount:</td>
                <td style="text-align: right; border-top:1px solid #000;"><?php echo number_format($total_discount, 2, '.', '');?></td>
            </tr>
            <?php 
        }//has total discount
        ?>
        ...
        {   
            ...
            ?>
            <tr style="font-size: 14px;">
                <td style="text-align: right;"><?php echo $row['PaymethodName'] . " : ";?></td>
        ...
        <tr style="font-size: 11px;">
            <td style="text-align: right;">Number of Items:</td>
...
--------------------------------------------------------
    ...
    Healthy Foods,Natural Goodness, Srilankan pride<br> 
