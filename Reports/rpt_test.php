                            ...
                            <div class="card-body">
                                <form action="../Controller/ReportController.php" class="form-inline" method="POST" accept-charset="utf-8">
                                    <div class="row">
                                        ...
                                        <div class="col-md-2 mt-4">
                                            <button type="submit" name="btn_test_sale_z_report" id="btn-filter" class="btn btn-success">Find</button>
                                        </div>
                                    </div>
                                </form>
                                <!-- Sales Table -->
                                <table class="table table-hover" id="tbl_sales">
                                    ...
                                    <?php 
                                        ...
                                        //top
                                        $sql = "SELECT min(InvEndTime) as StartTime FROM invoiceheader WHERE EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoiceheader.shop_SHID = ".$shop_id." AND invoiceheader.InvStat=1;";
                                        $dayStartData = $dbObj->getData($sql);
                                        $start_time = $dayStartData[0]['StartTime'];

                                        $sql = "SELECT max(InvEndTime) as EndTime FROM invoiceheader WHERE EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoiceheader.shop_SHID = ".$shop_id." AND invoiceheader.InvStat=1;";
                                        $dayEndData = $dbObj->getData($sql);
                                        $end_time = $dayEndData[0]['EndTime'];
                                        
                                        //================ Sale ================//
                                        $sql = "SELECT sum(GrossAmount) as GrossSale FROM invoiceheader WHERE EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoiceheader.shop_SHID = ".$shop_id." AND invoiceheader.InvStat=1;";
                                        ...
                                        $sql = "SELECT sum(DiscountAmount) as InvDiscount FROM invoiceheader WHERE EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."'AND invoiceheader.shop_SHID = ".$shop_id." AND invoiceheader.InvStat=1;";
                                        $saleData = $dbObj->getData($sql);
                                        $inv_discounts = $saleData[0]['InvDiscount'];

                                        $sql = "SELECT sum(NetAmount) as NetSale FROM invoiceheader WHERE EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoiceheader.shop_SHID = ".$shop_id." AND invoiceheader.InvStat=1;";
                                        $saleData = $dbObj->getData($sql);
                                        ...
                                        WHERE EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoiceheader.shop_SHID = ".$shop_id." AND ItemType='S' AND invoiceheader.InvStat=1;";
                                        $saleData = $dbObj->getData($sql);
                                        $service_amount = empty($saleData[0]['TotalService']) ? 0 : $saleData[0]['TotalService'];

                                        //line discounts
                                        $sql = "SELECT sum(lineDiscount) as LineDiscounts FROM invoiceheader WHERE EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND shop_SHID = ".$shop_id." AND invoiceheader.InvStat=1;";
                                        $saleData = $dbObj->getData($sql);
                                        ...
                                        WHERE creditsupplier.CreditStat = 1 AND EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND suppliers.shop_SHID = ".$shop_id.";";
