    ...
    <div class="body-wrapper">
        ...
        <div class="container-fluid">
            ...
            <div class="container-fluid">
                ...
                <input type="hidden" name="" id="title" value="Product Performance Report">
            
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title fw-semibold mb-2" style="margin-top: 0px;">Product Performance Report</h5>
                    </div>
                    <div class="card-body">
                        <div class="container">
                            <form action="../Controller/ReportController.php" method="POST" class="form-inline">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <button class="btn btn-success mt-4" name="btn_invoice_sales" id="btn_invoice_sales">Find</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <table id="tbl_inventory_summary">
                            ...
                            <tbody>
                                ...
                                foreach($invData as $row)
                                {
                                    ...
                                    //get sold data
                                    $sql_1 = "SELECT sum(SellQty) as sold_qty, sum(SoldAmount) as sold_amount FROM `invoicedetails` 
                                    INNER JOIN invoiceheader ON invoiceheader.IHID = invoicedetails.InvoiceHeader_IHID
                                    WHERE invoiceheader.EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND invoicedetails.products_PDID = ".$product_id.";";
                                    
                                    $soldData = $dbObj->getData($sql_1);
                                    $sold_qty = empty($soldData[0]['sold_qty']) ? 0 : floatval($soldData[0]['sold_qty']);
                                    $sold_amount = empty($soldData[0]['sold_amount']) ? 0 : floatval($soldData[0]['sold_amount']);

                                    //get return data
                                    $sql_2 = "SELECT sum(ReturnQty) as return_qty, sum(ReturnAmount) as return_amount FROM `returndetails` 
                                    INNER JOIN retrun_invoice_header ON retrun_invoice_header.RIHID = returndetails.ReturnHeader_RHID
                                    WHERE retrun_invoice_header.EffectiveDate BETWEEN '".$start_date."' AND '".$end_date."' AND returndetails.products_PDID = ".$product_id.";";

                                    $returnData = $dbObj->getData($sql_2);
                                    $return_qty = empty($returnData[0]['return_qty']) ? 0 : floatval($returnData[0]['return_qty']);
                                    $return_amount = empty($returnData[0]['return_amount']) ? 0 : floatval($returnData[0]['return_amount']);

                                    $net_sales = $sold_amount - $return_amount;
