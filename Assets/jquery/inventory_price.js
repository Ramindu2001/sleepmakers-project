$(document).ready(function() {
    ...
    var total_selling_sum = $('#total_selling_sum').val();
    var total_purchase_sum = $('#total_purchase_sum').val();
        ...
        buttons: [
                ...
                customize: function (win) {
                    ...
                    $(win.document.body).css('font-size', '10pt').append(
                        '<div style="display:flex; justify-content:end;">'+
                            ...
                            '<td>'+
                            '<p style="margin-bottom: 5px;"> ' + total_selling_sum + '</p>' +
                            '</td>'+
                            '<tr>' +
                            '</div>'+
                            '<div style="width:100%;">' +
                            ...
                            '<td>'+
                            '<p style="margin-bottom: 5px;"> ' + total_purchase_sum + '</p>' +
                            '</td>'+
                            '<tr>' +
                            '</div>'+
                            '</div>'
                    ...
                    // $(win.document.body).css('font-size', '10pt').append(
                    //         '<div style="text-align: center; margin-bottom: 20px;">' +
                    //         '<p style="margin-bottom: 5px;"> Total Selling Price' + total_selling_sum + '</p>' +
                    //         '</div>'
                    //     );
                 }
            },
            {   
                ...
                customize: function (xlsx) {
                    ...
                    // Corrected DOMParser to handle <row> tags
                    var parser = new DOMParser();
                    var xmlDoc = parser.parseFromString(newRows, 'text/xml');
                    var newRowElements = xmlDoc.getElementsByTagName('row');
